...
if(isset($_POST['place_order'])){
	...
	if($is_error==''){
		...
		$sql_orderid = "SELECT id FROM order_master WHERE id= (select MAX(id) from order_master)";
        $result_orderid = mysqli_query($con, $sql_orderid);

		$row_orderid = mysqli_fetch_assoc($result_orderid);
		$last_value_orderid = $row_orderid['id'];
		$update_value_orderid = $last_value_orderid + 1;//now update $update_value in database

		/* $insert_id=mysqli_insert_id($con); */

		$insert_id = $update_value_orderid;
